/// Human-readable labels for structured report metrics (review / future sync).
/// Keep keys in sync with `lib/metricLabels.ts`.
const Map<String, String> kReportingMetricLabels = {
  ...
  'tithes_offerings_seeds':
      'Tithes, offerings, seeds collected (break down totals)',
  ...
  'incident_log_detail':
      'Incident log detail (thefts, disruptions, arrests; refs)',
...
String? _labelForFallbackKey(String id) {
  if (id.endsWith('_primary_count')) return 'Primary count / volume';
  if (id.endsWith('_secondary_metric')) {
    return 'Secondary metric or % (optional)';
  }
  if (id.endsWith('_narrative')) {
    return 'Narrative, quality, risks, follow-up';
  }
  return null;
}

/// Label for a metric id stored on [Report.metrics] / web pipeline.
String displayLabelForMetricKey(String id) {
  final fixed = kReportingMetricLabels[id];
  if (fixed != null) return fixed;
  final fb = _labelForFallbackKey(id);
  if (fb != null) return fb;
  return _titleCaseSnake(id);
}

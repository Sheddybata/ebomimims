create or replace function public.manager_forward_report_to_director(p_report_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  if actor.id is null or actor.role <> 'manager' then
    raise exception 'Only managers can forward reports to directors';
  end if;
  ...
  if target.stage <> 'awaiting_manager' then
  ...
  set
    ...
    current_reviewer_role = 'director',
    returned_note = null,
    returned_by = null,
    returned_at = null,
    updated_at = now()
...
create or replace function public.manager_return_report_for_revision(
  p_report_id uuid,
  p_note text
)
returns void
...
begin
  select * into actor from public.profiles where id = auth.uid();
  if actor.id is null or actor.role <> 'manager' then
    raise exception 'Only managers can return reports for revision';
  end if;
...
grant execute on function public.manager_forward_report_to_director(uuid) to authenticated;
grant execute on function public.manager_return_report_for_revision(uuid, text) to authenticated;

create policy "managers can read same directorate reports"
